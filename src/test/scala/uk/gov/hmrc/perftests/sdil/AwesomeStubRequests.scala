...
object AwesomeStubRequests extends ServicesConfiguration {
  ...
  val baseUrlSDILReturns: String = baseUrlFor("soft-drinks-industry-levy-returns-frontend")
  val suffixURLSDILReturns: String  = "start"
  ...
  def updateUserRole(UTRValue: String): HttpRequestBuilder =
    http("Update current user to have IR-CT enrolment")
      .put(updateUserUrl)
      .body(StringBody(s"""{
  ...
  def postSuccessfulLogin: HttpRequestBuilder =
    http("Login with user credentials")
      ...
      .check(status.is(303))
      .check(header("Location").is(s"$baseUrlSDILReturns/$suffixURLSDILReturns": String))
