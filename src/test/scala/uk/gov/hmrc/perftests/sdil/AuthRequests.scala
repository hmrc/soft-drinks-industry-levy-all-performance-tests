...
object AuthRequests extends BaseRequest {
  ...
  def createAuthSession(utr: String): HttpRequestBuilder =
    http("Create Auth Session")
      .post(authWizUrl)
      .formParam("redirectionUrl", elAnyExpr(authSession))
      .formParam("credentialStrength", elAnyExpr("strong"))
      .formParam("authorityId", elAnyExpr(""))
      .formParam("confidenceLevel", elAnyExpr("50"))
      .formParam("affinityGroup", elAnyExpr("Individual"))
      .formParam("enrolment[0].name", elAnyExpr("IR-CT"))
      .formParam("enrolment[0].taxIdentifier[0].name", elAnyExpr("UTR"))
      .formParam("enrolment[0].taxIdentifier[0].value", elAnyExpr(utr))
      .formParam("enrolment[0].state", elAnyExpr("Activated"))
      .check(status.is(303))
